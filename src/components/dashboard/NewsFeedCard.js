...
                            <NewsItem key={item.id}>
                                {/* Backend doesn't provide image or url, so we remove these */}
                                {/* {item.image && <img src={item.image} alt={item.headline} />} */}
                                <div>
                                    {/* Use item.title for the main text, and make it a non-clickable span for now */}
                                    <span style={{ color: '#00adef', fontWeight: 'bold', display: 'block', fontSize: '1.05rem', lineHeight: '1.3' }}>
                                        {item.title} {/* Changed from item.headline */}
                                    </span>
                                    {/* Backend doesn't provide item.summary, so remove this paragraph */}
                                    {/* <p>{item.summary}</p> */}
                                    {/* Use item.date for the date */}
                                    <span>{item.source} • {item.date}</span> {/* Changed from item.datetime */}
                                </div>
                            </NewsItem>
                        ))
                    ) : (
