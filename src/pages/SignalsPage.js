...
function buildSignal(raw, index) {
    ...
    // Only calculate as fallback for legacy signals without stored entry
    ...
    // FIXED: Use stored SL/TP values from backend (set at signal creation)
    // Only calculate as fallback for legacy signals
    ...
    // Backend should provide real-time price updates
    const currentPrice = raw.currentPrice || raw.lastPrice || entry;
    const movePct = ((currentPrice - entry) / entry * 100);

    // Determine win/loss based on actual price vs levels
    let isWin = null;
    let resultText = null;
    if (expired || raw.status === 'closed') {
        // Check if any TP was hit during the trade
        if (raw.result) {
            isWin = raw.result === 'win' || raw.result === 'tp_hit';
            resultText = raw.resultText || (isWin ? 'Target Hit' : 'SL Hit');
        } else {
            // Fallback: check current price vs entry
            isWin = long ? currentPrice > entry : currentPrice < entry;
            if (isWin) {
                if (long ? currentPrice >= tp2 : currentPrice <= tp2) resultText = 'TP2 Hit';
                else if (long ? currentPrice >= tp1 : currentPrice <= tp1) resultText = 'TP1 Hit';
                else resultText = 'Partial Win';
            } else {
                resultText = 'SL Hit';
            }
        }
