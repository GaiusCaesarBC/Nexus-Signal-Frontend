...
const SignalDetailPage = () => {
    ...
    const buildDetail = (raw) => {
        ...
        // Determine win/loss based on actual data
        let isWin = null;
        let resultText = null;
        if (status === 'closed') {
            if (raw.outcome?.wasCorrect !== undefined) {
                isWin = raw.outcome.wasCorrect;
                resultText = isWin ? (currentPrice >= tp2 ? 'TP2 Hit' : 'TP1 Hit') : 'SL Hit';
            } else if (raw.result) {
                ...
                resultText = isWin ? 'Target Hit' : 'SL Hit';
            }
        }
