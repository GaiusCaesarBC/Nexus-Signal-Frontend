// client/src/pages/economicCalendar/AIMacroInsight.js
//
// Punchy AI macro insight (replaces generic prose).

import React from 'react';
import styled from 'styled-components';
import { Sparkles } from 'lucide-react';
import { useTheme } from '../../context/ThemeContext';
import { aiMacroInsight } from './derive';
...
const AIMacroInsight = ({ events }) => {
    const { theme } = useTheme();
    const insight = aiMacroInsight(events);

    return (
        <SectionCard theme={theme}>
            <SectionHeader>
                <SectionTitle theme={theme}>
                    <Sparkles size={14} /> AI Macro Insight
                </SectionTitle>
            </SectionHeader>
            <Body theme={theme}>
                <Sparkles size={18} />
